 ...
 *
 * @function Vsengine.Textures.Parsers.verifyCompressedTexture
 * @param {Vsengine.Types.Textures.CompressedTextureData} data - The compressed texture data to verify.
 * @since 3.80.0
...
var verifyCompressedTexture = function (data) {
    ...
    if (!checker) {
        console.warn(
            "No format checker found for internal format " +
                data.internalFormat +
                ". Assuming valid."
        );
        return true;
...
function check4x4(data) {
    ...
    for (var level = 0; level < mipmaps.length; level++) {
        ...
        if ((width << level) % 4 !== 0 || (height << level) % 4 !== 0) {
            console.warn(
                "BPTC, RGTC, and S3TC dimensions must be a multiple of 4 pixels, and each successive mip level must be half the size of the previous level, rounded down. Mip level " +
                    level +
                    " is " +
                    width +
                    "x" +
...
function checkPVRTC(data) {
    ...
    if (!IsSizePowerOfTwo(baseLevel.width, baseLevel.height)) {
        console.warn(
            "PVRTC base dimensions must be power of two. Base level is " +
                baseLevel.width +
                "x" +
                baseLevel.height
        );
...
function checkS3TCSRGB(data) {
    ...
    if (baseLevel.width % 4 !== 0 || baseLevel.height % 4 !== 0) {
        console.warn(
            "S3TC SRGB base dimensions must be a multiple of 4 pixels. Base level is " +
                baseLevel.width +
                "x" +
                baseLevel.height +
                " pixels"
