 ...
 *
 * @method Vsengine.GameObjects.Particles.Emitter#renderWebGL
 * @since 3.60.0
 * @private
 *
 * @param {Vsengine.Renderer.WebGL.WebGLRenderer} renderer - A reference to the current active WebGL renderer.
 * @param {Vsengine.GameObjects.Particles.ParticleEmitter} emitter - The Game Object being rendered in this call.
 ...
 */
var ParticleEmitterWebGLRenderer = function (
    renderer,
    emitter,
    camera,
    parentMatrix
    ...
    if (
        particleCount === 0 ||
        (viewBounds && !RectangleToRectangle(viewBounds, camera.worldView))
    ) {
        return;
    }
    ...
    camMatrix.multiplyWithOffset(
        managerMatrix,
        -camera.scrollX * emitter.scrollFactorX,
        -camera.scrollY * emitter.scrollFactorY
    );
