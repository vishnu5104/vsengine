 ...
 *
 * @method Vsengine.GameObjects.Mesh#renderWebGL
 * @since 3.0.0
 * @private
 *
 * @param {Vsengine.Renderer.WebGL.WebGLRenderer} renderer - A reference to the current active WebGL renderer.
 * @param {Vsengine.GameObjects.Mesh} src - The Game Object being rendered in this call.
...
var NineSliceWebGLRenderer = function (renderer, src, camera, parentMatrix) {
    ...
    var vertexOffset =
        pipeline.vertexCount * pipeline.currentShader.vertexComponentCount - 1;
    ...
    for (var i = 0; i < totalVerts; i++) {
        ...
        F32[++vertexOffset] = calcMatrix.getXRound(
            vert.vx,
            vert.vy,
            roundPixels
        );
        F32[++vertexOffset] = calcMatrix.getYRound(
            vert.vx,
            vert.vy,
            roundPixels
        );
        ...
        pipeline.currentBatch.count =
            pipeline.vertexCount - pipeline.currentBatch.start;
    }
