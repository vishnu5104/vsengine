 ...
 *
 * @method Vsengine.GameObjects.BitmapText#renderWebGL
 * @since 3.0.0
 * @private
 *
 * @param {Vsengine.Renderer.WebGL.WebGLRenderer} renderer - A reference to the current active WebGL renderer.
 * @param {Vsengine.GameObjects.BitmapText} src - The Game Object being rendered in this call.
...
var BitmapTextWebGLRenderer = function (renderer, src, camera, parentMatrix) {
    ...
    if (dropShadow) {
        ...
        var shadowTL = getTint(
            srcShadowColor,
            cameraAlpha * srcShadowAlpha * src._alphaTL
        );
        var shadowTR = getTint(
            srcShadowColor,
            cameraAlpha * srcShadowAlpha * src._alphaTR
        );
        var shadowBL = getTint(
            srcShadowColor,
            cameraAlpha * srcShadowAlpha * src._alphaBL
        );
        var shadowBR = getTint(
            srcShadowColor,
            cameraAlpha * srcShadowAlpha * src._alphaBR
        );
