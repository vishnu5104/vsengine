 ...
 *
 * @method Vsengine.GameObjects.DynamicBitmapText#renderCanvas
 * @since 3.0.0
 * @private
 *
 * @param {Vsengine.Renderer.Canvas.CanvasRenderer} renderer - A reference to the current active Canvas renderer.
 * @param {Vsengine.GameObjects.DynamicBitmapText} src - The Game Object being rendered in this call.
 ...
 */
var DynamicBitmapTextCanvasRenderer = function (
    renderer,
    src,
    camera,
    parentMatrix
    ...
    if (
        textLength === 0 ||
        !SetTransform(renderer, ctx, src, camera, parentMatrix)
    ) {
        return;
    }
    ...
    for (var i = 0; i < textLength; i++) {
        ...
        if (charCode === 10) {
            ...
            if (align === 1) {
                lineOffsetX =
                    (lineData.longest - lineData.lengths[currentLine]) / 2;
            } else if (align === 2) {
        ...
        xAdvance +=
            glyph.xAdvance +
            letterSpacing +
            (kerningOffset !== undefined ? kerningOffset : 0);
        lastGlyph = glyph;
