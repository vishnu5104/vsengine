 ...
 *
 * @method Vsengine.GameObjects.Rope#renderWebGL
 * @since 3.23.0
 * @private
 *
 * @param {Vsengine.Renderer.WebGL.WebGLRenderer} renderer - A reference to the current active WebGL renderer.
 * @param {Vsengine.GameObjects.Rope} src - The Game Object being rendered in this call.
...
var RopeWebGLRenderer = function (renderer, src, camera, parentMatrix) {
    ...
    for (var i = 0; i < meshVerticesLength; i += 2) {
        ...
        vertexViewU32[++vertexOffset] = getTint(
            colors[colorIndex],
            camera.alpha * (alphas[colorIndex] * alpha)
        );
