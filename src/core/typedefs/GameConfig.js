/**
 * @typedef {object} Vsengine.Types.Core.GameConfig
 * @since 3.0.0
 *
 ...
 * @property {number} [type=CONST.AUTO] - Which renderer to use. Vsengine.AUTO, Vsengine.CANVAS, Vsengine.HEADLESS, or Vsengine.WEBGL. AUTO picks WEBGL if available, otherwise CANVAS.
 ...
 * @property {HTMLCanvasElement} [canvas=null] - Provide your own Canvas element for Vsengine to use instead of creating one.
 * @property {string} [canvasStyle=null] - CSS styles to apply to the game canvas instead of Phasers default styles.
 * @property {boolean}[customEnvironment=false] - Is Vsengine running under a custom (non-native web) environment? If so, set this to `true` to skip internal Feature detection. If `true` the `renderType` cannot be left as `AUTO`.
 * @property {CanvasRenderingContext2D} [context] - Provide your own Canvas Context for Vsengine to use, instead of creating one.
 * @property {(Vsengine.Types.Scenes.SceneType|Vsengine.Types.Scenes.SceneType[])} [scene=null] - A scene or scenes to add to the game. If several are given, the first is started; the remainder are started only if they have `{ active: true }`. See the `sceneConfig` argument in `Vsengine.Scenes.SceneManager#add`.
 ...
 * @property {(boolean|Vsengine.Types.Core.InputConfig)} [input] - Input configuration, or `false` to disable all game input.
 ...
 * @property {(boolean|Vsengine.Types.Core.BannerConfig)} [banner=false] - Configuration for the banner printed in the browser console when the game starts.
 * @property {Vsengine.Types.Core.DOMContainerConfig} [dom] - The DOM Container configuration object.
 * @property {Vsengine.Types.Core.FPSConfig} [fps] - Game loop configuration.
 * @property {Vsengine.Types.Core.RenderConfig} [render] - Game renderer configuration.
 * @property {Vsengine.Types.Core.CallbacksConfig} [callbacks] - Optional callbacks to run before or after game boot.
 * @property {Vsengine.Types.Core.LoaderConfig} [loader] - Loader configuration.
 * @property {Vsengine.Types.Core.ImagesConfig} [images] - Images configuration.
 * @property {Vsengine.Types.Core.PhysicsConfig} [physics] - Physics configuration.
 * @property {Vsengine.Types.Core.PluginObject|Vsengine.Types.Core.PluginObjectItem[]} [plugins] - Plugins to install.
 * @property {Vsengine.Types.Core.ScaleConfig} [scale] - The Scale Manager configuration.
 * @property {Vsengine.Types.Core.AudioConfig} [audio] - The Audio Configuration object.
 * @property {Vsengine.Types.Core.PipelineConfig} [pipeline] - The WebGL Pipeline configuration object. Can also be part of the `RenderConfig`.
 ...
 * @property {Vsengine.Scale.ScaleModeType} [mode=Vsengine.Scale.ScaleModes.NONE] - The scale mode.
 ...
 * @property {Vsengine.Scale.CenterType} [autoCenter=Vsengine.Scale.Center.NO_CENTER] - Automatically center the canvas within the parent?
 * @property {number} [resizeInterval=500] - How many ms should elapse before checking if the browser size has changed?
 * @property {?(HTMLElement|string)} [fullscreenTarget] - The DOM element that will be sent into full screen mode, or its `id`. If undefined Vsengine will create its own div and insert the canvas into it when entering fullscreen mode.
