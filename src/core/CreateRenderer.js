...
/**
 * Called automatically by Vsengine.Game and responsible for creating the renderer it will use.
 *
 * Relies upon two webpack global flags to be defined: `WEBGL_RENDERER` and `CANVAS_RENDERER` during build time, but not at run-time.
 *
 * @function Vsengine.Core.CreateRenderer
 * @since 3.0.0
 *
 * @param {Vsengine.Game} game - The Vsengine.Game instance on which the renderer will be set.
 */
var CreateRenderer = function (game) {
    ...
    if (
        ...
        if (config.renderType === CONST.WEBGL) {
            ...
            throw new Error(
                "Unknown value for renderer type: " + config.renderType
            );
        }
    }
