 ...
 *
 * This method is called by `Vsengine.Physics.Arcade.SeparateY` and should not be
 * called directly.
 *
 * @function Vsengine.Physics.Arcade.ProcessY.Set
 * @ignore
 * @since 3.50.0
 *
 * @param {Vsengine.Physics.Arcade.Body} b1 - The first Body to separate.
 * @param {Vsengine.Physics.Arcade.Body} b2 - The second Body to separate.
...
var Set = function (b1, b2, ov) {
    ...
    body1OnTop =
        Math.abs(body1.bottom - body2.y) <= Math.abs(body2.bottom - body1.y);
 ...
 *
 * @function Vsengine.Physics.Arcade.ProcessY.BlockCheck
 * @ignore
 * @since 3.50.0
 *
 * @return {number} The BlockCheck result. 0 = not blocked. 1 = Body 1 blocked. 2 = Body 2 blocked.
 ...
 *
 * @function Vsengine.Physics.Arcade.ProcessY.Check
 * @ignore
 * @since 3.50.0
 *
...
var Check = function () {
    ...
    var nv1 =
        Math.sqrt((v2 * v2 * body2.mass) / body1.mass) * (v2 > 0 ? 1 : -1);
    var nv2 =
        Math.sqrt((v1 * v1 * body1.mass) / body2.mass) * (v1 > 0 ? 1 : -1);
    var avg = (nv1 + nv2) * 0.5;
 ...
 * The main check function. Runs through one of the four possible tests and returns the results.
 *
 * @function Vsengine.Physics.Arcade.ProcessY.Run
 * @ignore
 * @since 3.50.0
 *
 ...
 *
 * @function Vsengine.Physics.Arcade.ProcessY.RunImmovableBody1
 * @ignore
 * @since 3.50.0
 *
...
var RunImmovableBody1 = function (blockedState) {
    ...
    if (body1.moves) {
        var body1Distance = body1.directControl
            ? body1.x - body1.autoFrame.x
            : body1.x - body1.prev.x;
 ...
 *
 * @function Vsengine.Physics.Arcade.ProcessY.RunImmovableBody2
 * @ignore
 * @since 3.50.0
 *
 * @param {number} blockedState - The block state value.
...
var RunImmovableBody2 = function (blockedState) {
    ...
    //  This is special case code that handles things like horizontally moving platforms you can ride
    if (body2.moves) {
        var body2Distance = body2.directControl
            ? body2.x - body2.autoFrame.x
            : body2.x - body2.prev.x;
...
/**
 * @namespace Vsengine.Physics.Arcade.ProcessY
 * @ignore
 */
