 ...
 *
 * These are created when you use the Vsengine.Loader.LoaderPlugin#texture method and are not typically created directly.
 *
 * For documentation about what all the arguments and configuration options mean please see Vsengine.Loader.LoaderPlugin#texture.
 *
 * @class CompressedTextureFile
 * @extends Vsengine.Loader.MultiFile
 * @memberof Vsengine.Loader.FileTypes
 * @constructor
 * @since 3.60.0
 *
 * @param {Vsengine.Loader.LoaderPlugin} loader - A reference to the Loader that is responsible for this file.
 * @param {string} key - The key to use for this file.
 * @param {Vsengine.Types.Loader.FileTypes.CompressedTextureFileEntry} entry - The compressed texture file entry to load.
 * @param {Vsengine.Types.Loader.XHRSettingsObject} [xhrSettings] - Extra XHR Settings specifically for this file.
 */
var CompressedTextureFile = new Class({
    Extends: MultiFile,

    initialize: function CompressedTextureFile(
        loader,
        key,
        entry,
        xhrSettings
        ...
        if (entry.multiAtlasURL) {
            ...
            var extension = entry.textureURL.substr(
                entry.textureURL.length - 3
            );
     ...
     *
     * @method Vsengine.Loader.FileTypes.CompressedTextureFile#onFileComplete
     * @since 3.60.0
     *
     * @param {Vsengine.Loader.File} file - The File that has completed processing.
     */
            ...
            if (file.type === "json" && file.data.hasOwnProperty("textures")) {
                ...
                for (var i = 0; i < textures.length; i++) {
                    ...
                    if (textures[i].normalMap) {
                        var normalMap = new BinaryFile(
                            loader,
                            key,
                            textures[i].normalMap,
                            textureXhrSettings
                        );
     ...
     *
     * @method Vsengine.Loader.FileTypes.CompressedTextureFile#addToCache
     * @since 3.60.0
     */
    addToCache: function () {
        function compressionWarning(message) {
            console.warn(
                'Compressed Texture Invalid: "' + image.key + '". ' + message
            );
        }
        ...
        if (this.isReadyToProcess()) {
            ...
            if (entry.multiAtlasURL) {
                ...
                if (entry.type === "PVR") {
                    ...
                    if (!textureData) {
                        compressionWarning(
                            "KTX file contains unsupported format."
                        );
                    }
                }
                ...
                if (textureData && !verifyCompressedTexture(textureData)) {
                    compressionWarning(
                        "Texture dimensions failed verification. Check the texture format specifications for " +
                            entry.format +
                            " 0x" +
                            textureData.internalFormat.toString(16) +
                            "."
                    );
                    textureData = null;
                }
                ...
                if (
                    textureData &&
                    !renderer.supportsCompressedTexture(
                        entry.format,
                    ...
                    compressionWarning(
                        "Texture format " +
                            ...
                            textureData.internalFormat +
                            " not supported by the GPU. Texture invalid. This is often due to the texture using sRGB instead of linear RGB."
                    );
                    textureData = null;
                }

                if (textureData) {
                    textureData.format = renderer.getCompressedTextureName(
                        entry.format,
                        textureData.internalFormat
                    );
     ...
     *
     * @method Vsengine.Loader.FileTypes.CompressedTextureFile#addMultiToCache
     * @since 3.60.0
     */
 ...
 * ```
 *
 * See the documentation for `Vsengine.Types.Loader.FileTypes.CompressedTextureFileConfig` for more details.
 *
 ...
 *
 * Vsengine supports loading both the PVR and KTX container formats. Within those, it can parse
 ...
 * The typical flow for a Vsengine Scene is that you load assets in the Scene's `preload` method and then when the
 ...
 *
 * Unlike other file loaders in Vsengine, the URLs must include the file extension.
 *
 * Note: The ability to load this type of file will only be available if the Compressed Texture File type has been built into Vsengine.
 * It is available in the default build but can be excluded from custom builds.
 *
 * @method Vsengine.Loader.LoaderPlugin#texture
 * @fires Vsengine.Loader.Events#ADD
 * @since 3.60.0
 *
 * @param {(string|Vsengine.Types.Loader.FileTypes.CompressedTextureFileConfig|Vsengine.Types.Loader.FileTypes.CompressedTextureFileConfig[])} key - The key to use for this file, or a file configuration object, or array of them.
 * @param {Vsengine.Types.Loader.FileTypes.CompressedTextureFileConfig} [url] - The compressed texture configuration object. Not required if passing a config object as the `key` parameter.
 * @param {Vsengine.Types.Loader.XHRSettingsObject} [xhrSettings] - An XHR Settings configuration object. Used in replacement of the Loaders default XHR Settings.
 *
...
FileTypesManager.register("texture", function (key, url, xhrSettings) {
    ...
    var AddEntry = function (loader, key, urls, xhrSettings) {
        ...
        if (!matched) {
            console.warn(
                "No supported compressed texture format or IMG fallback",
                key
            );
            ...
            var texture = new CompressedTextureFile(
                loader,
                key,
                entry,
                xhrSettings
