 ...
 *
 * @function Vsengine.Renderer.Snapshot.WebGL
 * @since 3.0.0
 *
 * @param {WebGLRenderingContext} sourceContext - The WebGL context to take a snapshot of.
 * @param {Vsengine.Types.Renderer.Snapshot.SnapshotState} config - The snapshot configuration object.
 */
var WebGLSnapshot = function (sourceContext, config) {
    ...
    var bufferWidth = isFramebuffer
        ? GetFastValue(config, "bufferWidth", 1)
        : gl.drawingBufferWidth;
    var bufferHeight = isFramebuffer
        ? GetFastValue(config, "bufferHeight", 1)
    ...
    if (getPixel) {
        ...
        for (var py = 0; py < height; py++) {
            for (var px = 0; px < width; px++) {
                ...
                var destIndex = isFramebuffer
                    ? total - (py * width + (width - px)) * 4
                    : (py * width + px) * 4;
