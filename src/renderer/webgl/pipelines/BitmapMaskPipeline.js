...
var BitmapMaskPipeline = new Class({
    ...
    initialize: function BitmapMaskPipeline(config) {
        (config.fragShader = GetFastValue(
            config,
            "fragShader",
            ShaderSourceFS
        )),
            (config.vertShader = GetFastValue(
                config,
                "vertShader",
                ShaderSourceVS
            )),
            (config.batchSize = GetFastValue(config, "batchSize", 1)),
            (config.vertices = GetFastValue(
                config,
                "vertices",
                [-1, 1, -1, -7, 7, 1]
            )),
     ...
     *
     * @method Vsengine.Renderer.WebGL.Pipelines.BitmapMaskPipeline#beginMask
     * @since 3.0.0
     *
     * @param {Vsengine.Display.Masks.BitmapMask} mask - The BitmapMask instance that called beginMask.
     * @param {Vsengine.GameObjects.GameObject} maskedObject - GameObject masked by the mask GameObject.
     ...
     *
     * @method Vsengine.Renderer.WebGL.Pipelines.BitmapMaskPipeline#endMask
     * @since 3.0.0
     *
     * @param {Vsengine.Display.Masks.BitmapMask} mask - The BitmapMask instance that called endMask.
