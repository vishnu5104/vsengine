 ...
 * Creates a new Tween.
 *
 * @function Vsengine.Tweens.Builders.TweenBuilder
 * @since 3.0.0
 *
 * @param {Vsengine.Tweens.TweenManager} parent - The owner of the new Tween.
 * @param {Vsengine.Types.Tweens.TweenBuilderConfig|object} config - Configuration for the new Tween.
 * @param {Vsengine.Types.Tweens.TweenConfigDefaults} defaults - Tween configuration defaults.
 *
 * @return {Vsengine.Tweens.Tween} The new tween.
 */
var TweenBuilder = function (parent, config, defaults) {
    ...
    var addTarget = function (tween, targetIndex, key, value) {
        if (key === "texture") {
            ...
            var interpolationFunc = GetInterpolationFunction(
                GetValue(value, "interpolation", interpolation)
            );
            ...
            tween.add(
                ...
                ops.getActive,
                GetEaseFunction(
                    GetValue(value, "ease", ease),
                    GetValue(value, "easeParams", easeParams)
                ),
                GetNewValue(value, "delay", delay),
                GetValue(value, "duration", duration),
    ...
    for (var p = 0; p < props.length; p++) {
        ...
        for (var targetIndex = 0; targetIndex < targets.length; targetIndex++) {
            ...
            if (
                key === "scale" &&
                !targets[targetIndex].hasOwnProperty("scale")
            ) {
