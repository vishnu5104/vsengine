 ...
 *
 * @function Vsengine.Tilemaps.Components.HexagonalGetTileCorners
 * @since 3.60.0
 *
 ...
 *
 * @return {Vsengine.Math.Vector2[]} An array of Vector2s corresponding to the world XY location of each tile corner.
 */
var HexagonalGetTileCorners = function (tileX, tileY, camera, layer) {
    ...
    for (var i = 0; i < 6; i++) {
        ...
        corners.push(
            new Vector2(
                center.x + hexWidth * Math.cos(angle),
                center.y + hexHeight * Math.sin(angle)
            )
        );
    }
