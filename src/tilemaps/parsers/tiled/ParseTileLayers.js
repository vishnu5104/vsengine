 ...
 *
 * @function Vsengine.Tilemaps.Parsers.Tiled.ParseTileLayers
 * @since 3.0.0
 *
 * @param {object} json - The Tiled JSON object.
 * @param {boolean} insertNull - Controls how empty tiles, tiles with an index of -1, in the map
 * data are handled (see {@link Vsengine.Tilemaps.Parsers.Tiled.ParseJSONTiled}).
 *
 * @return {Vsengine.Tilemaps.LayerData[]} - An array of LayerData objects, one for each entry in
    ...
    while (
        ...
        if (curl.type !== "tilelayer") {
            if (curl.type === "group") {
                ...
                var nextGroupState = CreateGroupLayer(
                    json,
                    curl,
                    curGroupState
                );
