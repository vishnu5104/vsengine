 ...
 *
 * @function Vsengine.Tilemaps.Parsers.Tiled.ParseTilesets
 * @since 3.0.0
 *
            ...
            if (json.version > 1) {
                ...
                if (Array.isArray(set.tiles)) {
                    ...
                    for (var t = 0; t < set.tiles.length; t++) {
                        ...
                        if (tile.objectgroup) {
                            (
                                datas[tile.id] || (datas[tile.id] = {})
                            ).objectgroup = tile.objectgroup;

                            if (tile.objectgroup.objects) {
                                var parsedObjects2 =
                                    tile.objectgroup.objects.map(function (
                                        obj
                                    ) {
                                        return ParseObject(obj);
                                    });

                                datas[tile.id].objectgroup.objects =
                                    parsedObjects2;
                            }
                        }
                ...
                if (set.tiles) {
                    ...
                    // Parse the objects into Vsengine format to match handling of other Tiled objects
