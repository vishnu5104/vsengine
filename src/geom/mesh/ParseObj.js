...
function parsePolygon(lineItems, result) {
    ...
    for (var i = 0; i < totalVertices; i++) {
        ...
        if (vertexIndex !== 0) {
            ...
            if (vertexIndex < 0) {
                vertexIndex =
                    currentModel(result).vertices.length + 1 + vertexIndex;
            }
 ...
 *
 * @function Vsengine.Geom.Mesh.ParseObj
 * @since 3.50.0
 *
 * @param {string} data - The OBJ File data as a raw string.
 * @param {boolean} [flipUV=true] - Flip the UV coordinates?
 *
 * @return {Vsengine.Types.Geom.Mesh.OBJData} The parsed model and material data.
 */
