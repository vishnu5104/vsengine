 ...
 *
 * @function Vsengine.Geom.Rectangle.RandomOutside
 * @since 3.10.0
 *
 * @generic {Vsengine.Geom.Point} O - [out,$return]
 *
 * @param {Vsengine.Geom.Rectangle} outer - The outer Rectangle to get the random point within.
 * @param {Vsengine.Geom.Rectangle} inner - The inner Rectangle to exclude from the returned point.
 * @param {Vsengine.Geom.Point} [out] - A Point, or Point-like object to store the result in. If not specified, a new Point will be created.
 *
 * @return {Vsengine.Geom.Point} A Point object containing the random values in its `x` and `y` properties.
 */
var RandomOutside = function (outer, inner, out) {
    ...
    if (ContainsRect(outer, inner)) {
        ...
        switch (Between(0, 3)) {
            ...
            case 3: // Right
                out.x =
                    inner.right + Math.random() * (outer.right - inner.right);
