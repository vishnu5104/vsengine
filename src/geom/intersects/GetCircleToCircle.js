 ...
 *
 * @function Vsengine.Geom.Intersects.GetCircleToCircle
 * @since 3.0.0
 *
 * @param {Vsengine.Geom.Circle} circleA - The first Circle to check for intersection.
 * @param {Vsengine.Geom.Circle} circleB - The second Circle to check for intersection.
        ...
        if (y0 === y1) {
            ...
            if (lambda === 0) {
                ...
                out.push(
                    new Point(
                        x,
                        (-coefficientB + Math.sqrt(lambda)) / (2 * coefficientA)
                    )
                );
                out.push(
                    new Point(
            ...
            var v1 = (x0 - x1) / (y0 - y1);
            var n =
                (r1 * r1 - r0 * r0 - x1 * x1 + x0 * x0 - y1 * y1 + y0 * y0) /
                (2 * (y0 - y1));
