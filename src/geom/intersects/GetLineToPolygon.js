 ...
 *
 * @function Vsengine.Geom.Intersects.GetLineToPolygon
 * @since 3.50.0
 *
 * @param {Vsengine.Geom.Line} line - The line segment, or ray, to check. If a ray, set the `isRay` parameter to `true`.
 * @param {Vsengine.Geom.Polygon | Vsengine.Geom.Polygon[]} polygons - A single polygon, or array of polygons, to check.
 * @param {boolean} [isRay=false] - Is `line` a ray or a line segment?
 * @param {Vsengine.Math.Vector4} [out] - A Vector4 to store the intersection results in.
 *
 * @return {Vsengine.Math.Vector4} A Vector4 containing the intersection results, or `null`.
 */
