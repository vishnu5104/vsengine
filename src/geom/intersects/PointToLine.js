 ...
 *
 * @function Vsengine.Geom.Intersects.PointToLine
 * @since 3.0.0
 *
 * @param {(Vsengine.Geom.Point|any)} point - The point, or point-like object to check.
 * @param {Vsengine.Geom.Line} line - The line segment to test for intersection on.
...
var PointToLine = function (point, line, lineThickness) {
    ...
    if (r < 0) {
        //  Outside line1
        return (
            Math.sqrt((x1 - px) * (x1 - px) + (y1 - py) * (y1 - py)) <=
            lineThickness
        );
    } else if (r >= 0 && r <= 1) {
        //  On the line segment
        ...
        //  Outside line2
        return (
            Math.sqrt((x2 - px) * (x2 - px) + (y2 - py) * (y2 - py)) <=
            lineThickness
        );
    }
};
