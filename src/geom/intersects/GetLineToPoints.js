 ...
 *
 * @function Vsengine.Geom.Intersects.GetLineToPoints
 * @since 3.50.0
 *
 * @param {Vsengine.Geom.Line} line - The line segment, or ray, to check. If a ray, set the `isRay` parameter to `true`.
 * @param {Vsengine.Math.Vector2[] | Vsengine.Geom.Point[]} points - An array of points to check.
 * @param {boolean} [isRay=false] - Is `line` a ray or a line segment?
 * @param {Vsengine.Math.Vector3} [out] - A Vector3 to store the intersection results in.
 *
 * @return {Vsengine.Math.Vector3} A Vector3 containing the intersection results, or `null`.
 */
