 ...
 *
 * You can install this plugin into your Vsengine game by either importing it, if you're using ES6:
 *
 * ```javascript
 * import * as SpinePlugin from './SpinePlugin.js';
 * ```
 *
 * and then adding it to your Vsengine Game configuration:
 *
 * ```javascript
 * plugins: {
 *     scene: [
 ...
 * Vsengine Loader. However, doing so will not add it to the current Scene. It will be available from any
 * subsequent Scenes.
 *
 ...
 *
 * Vsengine expects the Spine data to be exported from the Spine application in a JSON format, not binary.
 ...
 *
 * When destroying a Vsengine Game instance, if you need to re-create it again on the same page without
 ...
 *
 * @class SpinePlugin
 * @extends Vsengine.Plugins.ScenePlugin
 * @constructor
 * @since 3.19.0
 *
 * @param {Vsengine.Scene} scene - A reference to the Scene that has installed this plugin.
 * @param {Vsengine.Plugins.PluginManager} pluginManager - A reference to the Vsengine Plugin Manager.
...
var SpinePlugin = new Class({
    ...
    initialize: function SpinePlugin(scene, pluginManager, pluginKey) {
        ...
        /**
         * The underlying WebGL context of the Vsengine renderer.
         *
        ...
        var addContainer = function (x, y, children) {
            var spinePlugin = this.scene.sys[pluginKey];
            var spineGO = new SpineContainer(
                this.scene,
                spinePlugin,
                x,
                y,
        ...
        var makeContainer = function (config, addToScene) {
            ...
            var spinePlugin = this.scene.sys[pluginKey];
            var container = new SpineContainer(
                this.scene,
                spinePlugin,
                x,
                y,
     ...
     */
    bootCanvas: function () {
        this.skeletonRenderer = new Spine.canvas.SkeletonRenderer(
    ...
    bootWebGL: function () {
        ...
        if (!sceneRenderer) {
            sceneRenderer = new Spine.webgl.SceneRenderer(
    ...
    getAtlasCanvas: function (key) {
        ...
        if (spineTextures.has(key)) {
            ...
            atlas = new this.runtime.TextureAtlas(atlasEntry.data, function (
                path
            ) {
                return new Spine.canvas.CanvasTexture(
                    textures.get(atlasEntry.prefix + path).getSourceImage()
                );
            });
        }
    ...
    getAtlasWebGL: function (key) {
        ...
        if (spineTextures.has(key)) {
            ...
            atlas = new this.runtime.TextureAtlas(atlasEntry.data, function (
                ...
                return new Spine.webgl.GLTexture(
                    gl,
                    textures.get(atlasEntry.prefix + path).getSourceImage(),
                    false
                );
     ...
     * its events to know when it's safe to use the asset. Please see the Vsengine.Loader.LoaderPlugin class for more details.
     *
     * Vsengine expects the Spine data to be exported from the Spine application in a JSON format, not binary. The associated
     ...
     * ```
     *
     * See the documentation for `Vsengine.Types.Loader.FileTypes.SpineFileConfig` for more details.
     *
     ...
     *
     * Note: The ability to load this type of file will only be available if the Spine Plugin has been built or loaded into Vsengine.
     *
     * @method Vsengine.Loader.LoaderPlugin#spine
     * @fires Vsengine.Loader.Events#ADD
     * @since 3.19.0
     *
     * @param {(string|Vsengine.Types.Loader.FileTypes.JSONFileConfig|Vsengine.Types.Loader.FileTypes.JSONFileConfig[])} key - The key to use for this file, or a file configuration object, or array of them.
     ...
     * @param {Vsengine.Types.Loader.XHRSettingsObject} [textureXhrSettings] - An XHR Settings configuration object for the Spine json file. Used in replacement of the Loaders default XHR Settings.
     * @param {Vsengine.Types.Loader.XHRSettingsObject} [atlasXhrSettings] - An XHR Settings configuration object for the Spine atlas file. Used in replacement of the Loaders default XHR Settings.
     * @param {object} [settings] - An external Settings configuration object { prefix: '' }
     *
     * @return {Vsengine.Loader.LoaderPlugin} The Loader instance.
     */
    ...
    worldToLocal: function (x, y, skeleton, bone) {
        ...
        if (bone && bone.parent !== null) {
            bone.parent.worldToLocal(
                temp2.set(temp1.x - skeleton.x, temp1.y - skeleton.y, 0)
            );

            return new this.runtime.Vector2(temp2.x, temp2.y);
        } else if (bone) {
            return new this.runtime.Vector2(
                temp1.x - skeleton.x,
                temp1.y - skeleton.y
            );
        } else {
    ...
    createSkeleton: function (key, skeletonJSON) {
        ...
        if (!this.spineTextures.has(atlasKey)) {
            ...
            for (i = 0; i < atlas.pages.length; i++) {
                ...
                realTextureKey = atlasData.prefix
                    ? atlasData.prefix + atlasPage.name
                    : atlasPage.name;
                if (this.isWebGL) {
                    atlasPage.setTexture(
                        new this.runtime.GLTexture(
                            gl,
                    ...
                    atlasPage.setTexture(
                        new this.runtime.CanvasTexture(
                            this.textures.get(realTextureKey).getSourceImage()
                        )
                    );
                }
