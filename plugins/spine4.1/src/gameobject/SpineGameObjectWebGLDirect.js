 ...
 */
var SpineGameObjectWebGLDirect = function (
    renderer,
    src,
    camera,
    parentMatrix,
    ...
    if (src.scaleX < 0) {
        ...
        //  -180 degrees to account for the difference in Spine vs. Vsengine rotation when inversely scaled
        src.root.rotation = Wrap(
            RadToDeg(calcMatrix.rotationNormalized) - 180,
            0,
            360
        );
    } else {
        ...
        src.root.rotation = Wrap(
            RadToDeg(CounterClockwise(calcMatrix.rotationNormalized)) + 90,
            0,
            360
        );
    }
