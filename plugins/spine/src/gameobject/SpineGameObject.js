...
/**
 * @classdesc
 * A Spine Game Object is a Vsengine level object that can be added to your Vsengine Scenes. It encapsulates
 ...
 * You can only create them if the Spine plugin has been loaded into Vsengine.
 *
 ...
 * @since 3.19.0
 *
 * @param {Vsengine.Scene} scene - A reference to the Scene that this Game Object belongs to.
 * @param {SpinePlugin} pluginManager - A reference to the Vsengine Spine Plugin.
...
var SpineGameObject = new Class({
    ...
    willRender: function (camera, container) {
        ...
        var result =
            !this.skeleton ||
            !(
                GameObjectRenderMask !== this.renderFlags ||
    ...
    setSkeleton: function (atlasDataKey, animationName, loop, skeletonJSON) {
        ...
        if (this.root) {
            //  +90 degrees to account for the difference in Spine vs. Vsengine rotation
    ...
    updateSize: function () {
        ...
        this.displayOriginY =
            this.y - (height - (this.height + bounds.offset.y));
    ...
    setAttachment: function (slotName, attachmentName) {
        if (
            ...
            slotName.length === attachmentName.length
        ) {
    ...
    angleBoneToXY: function (bone, worldX, worldY, offset, minAngle, maxAngle) {
        ...
        var angle = CounterClockwise(
            AngleBetween(bone.worldX, height - bone.worldY, worldX, worldY) +
                DegToRad(offset)
        );
